...
const Timeline = ({ id, setIsModalNewTaskOpen }: Props) => {
  ...
  // reformat the tasks object to be compatible with the Gantt task component required by the library
  const ganttTasks = useMemo(() => {
    return (
      tasks?.map((task) => ({
        ...
        name: task.title,
        id: `Task-${task.id}`,
        type: "task" as TaskTypeItems, // type of the task
        progress: task.points ? (task.points / 10) * 100 : 0, // convert points to progress percentage
        isDisabled: false,
      })) || []
    );
  }, [tasks]);

  // handle the view mode change based on the selected option (day, week, month)
